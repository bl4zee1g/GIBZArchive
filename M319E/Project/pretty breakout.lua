...
local function show(node)
	os.execute(
		'gum style --border rounded --margin "1 0" --padding "1 2" --border-foreground 212 --width 100 -- '
			.. shell_quote(node.text)
	)
end
...
local function choose(node)
	...
	local cmd = 'gum choose --header "What do you do?" --cursor "> "'
	for _, c in ipairs(node.choices) do
		cmd = cmd .. " " .. shell_quote(c[2])
	end

	local handle = io.popen(cmd)
	local picked = handle:read("*l")
	handle:close()
...
os.execute(
	'gum style --border double --align center --width 100 --margin "1 0" --foreground 212 -- '
		.. shell_quote("──୨ৎ──── Mamdanistan breakout ────୨ৎ────")
)
local id = "start"
while id do
	local node = Story[id]
...
os.execute("gum style --align center --width 50 --faint -- " .. shell_quote("— The End —"))
